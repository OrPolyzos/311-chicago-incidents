...
declare
  sr_record record;
begin
  for sr_record in (select date_trunc('day', creation_date_time) :: timestamp without time zone as sr_day, count(*) as sr_count
                    from chicago_service_requests.service_requests
                    where sr_type = input_sr_type
                      and creation_date_time between input_start_date and input_end_date
                    group by sr_day)
  loop
...
declare
  sr_record record;
begin
  for sr_record in (select outer_table.zip_code, outer_table.sr_type, count(outer_table.sr_type) as occurences
                    ...
                    having outer_table.sr_type =
                           (select inner_table.sr_type
                            ...
                            where inner_table.zip_code = outer_table.zip_code
                              and inner_table.creation_date_time = outer_table.creation_date_time
...
declare
  sr_record record;
begin
  for sr_record in (select sr_type, avg(extract(epoch from (completion_date_time :: timestamp - creation_date_time :: timestamp))) as sr_avg
                    from chicago_service_requests.service_requests
                    where completion_date_time notnull
                      and creation_date_time between input_start_date and input_end_date
                    group by sr_type)
  loop
    out_sr_type := sr_record.sr_type;
...
begin
  for sr_record in (select sr_type, count(*) as sr_count
                    ...
                    where input_date = creation_date_time
                      and public.ST_Contains(public.ST_AsText(public.ST_Envelope(
                                                                ('POLYGON((' || input_min_x || ' ' || input_min_y || ', ' || input_min_x || ' ' || input_max_y || ', ' ||input_max_x || ' ' ||
                                                                 input_max_y || ', ' || input_max_x || ' ' || input_min_y || ', ' || input_min_x || ' ' || input_min_y || '))'))),
...
declare
  sr_record record;
begin
  for sr_record in (select ssa, count(*) as ssa_count
                    from chicago_service_requests.service_requests as t1,
                         (select gc.service_request_id as service_request_id, gc.special_service_area_id as special_service_area_id, ssat.ssa as ssa
                          from chicago_service_requests.garbage_carts_requests as gc,
                               chicago_service_requests.special_service_area as ssat
                          where gc.special_service_area_id = ssat.id
                          union all
                          select ab.service_request_id as service_request_id, ab.special_service_area_id as special_service_area_id, ssat.ssa as ssa
                          from chicago_service_requests.abandoned_vehicle_requests as ab,
                               chicago_service_requests.special_service_area as ssat
                          where ab.special_service_area_id = ssat.id
                          union all
                          select ab.service_request_id as service_request_id, ab.special_service_area_id as special_service_area_id, ssat.ssa as ssa
                          from chicago_service_requests.rodent_baiting_requests as ab,
                               chicago_service_requests.special_service_area as ssat
                          where ab.special_service_area_id = ssat.id
                         --union all
...
declare
  sr_record record;
begin
  for sr_record in (select licence_plate, count(*) as sr_count from chicago_service_requests.abandoned_vehicle_requests group by licence_plate having count(*) > 1)
  loop
...
declare
  sr_record record;
begin
  for sr_record in (select vehicle_color, vc_count, col_rank
                    from (select vehicle_color, vc_count, dense_rank() over (order by vc_count desc) as col_rank
                          from (select vehicle_color, count(vehicle_color) as vc_count from chicago_service_requests.abandoned_vehicle_requests group by vehicle_color) t1) t2
                    where col_rank = 2)
  loop
